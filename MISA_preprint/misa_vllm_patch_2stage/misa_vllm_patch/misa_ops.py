...
def misa_page_mqa_logits(
    ...
    stage_1_topk = min(stage1_topk, max_model_len)
    stage_1_logits = paged_mqa_sparse_head_return_logits_interface(
        q=sparse_head_q,
        kv_cache=kv_cache,
    ...
    sparse_topk_indices = stage_1_logits.topk(k=stage_1_topk, dim=-1, largest=True, sorted=False).indices.to(torch.int32)  # [batch, seq_len, stage_1_topk]
    logits = paged_mqa_attn_return_logits_interface(
        q=q,
        kv_cache=kv_cache,
        weights=weights,
        context_lens=context_lens,
    ...
    return logits.view(-1, stage_1_topk), sparse_topk_indices.view(-1, stage_1_topk)
...
def misa_mqa_sparse_head_return_logits(
    ...
    def misa_mqa_sparse_head_return_logits_kernel(
        ...
        with T.Kernel(T.ceildiv(seq_len, block_Q), threads=threads) as bx:
            ...
            for b_i in T.serial(T.ceildiv(cu_seq, block_N)):
...
def misa_mqa_logits(
    ...
    seq_len_k = k.shape[0]
    stage_1_topk = min(stage1_topk, seq_len_k)
    stage_1_logits = misa_mqa_sparse_head_return_logits_interface(q=sparse_head_q, kv=k, weights=sparse_head_weights, cu_seqlen_ks=cu_seqlen_ks, cu_seqlen_ke=cu_seqlen_ke)
    sparse_topk_indices = stage_1_logits.topk(k=stage_1_topk, dim=-1, largest=True, sorted=False).indices.to(torch.int32)  # [M, 8192]
    logits = misa_mqa_attn_return_logits_interface(q=q, kv=k, weights=weights, sparse_topk_indices=sparse_topk_indices, cu_seqlen_ks=cu_seqlen_ks, cu_seqlen_ke=cu_seqlen_ke)  # [M, 8192]

    return logits, sparse_topk_indices