...
class MiSAIndexer(nn.Module):
    ...
    def __init__(self,
        ...
        if self.use_misa:
            ...
            self.stage1_topk = getattr(config, "misa_stage1_topk", None)  # 32768
            ...
            if 'layers.0' in prefix:
                print(prefix)
                print(f'Using misa indexer with topk_heads {self.topk_heads}, decode_topk_heads {self.decode_topk_heads}, stage1_topk {self.stage1_topk}.')
                if self.use_qnorm:
    ...
    def forward(self, hidden_states: torch.Tensor, qr: torch.Tensor, positions,
        ...
        if self.use_misa and self.use_qnorm:
            # 1. 获取 top-k heads 的索引
            # weights: (seq, heads) -> 沿 heads 维度取 topk
            # topk_indices: (seq, topk_heads)
            # weights, topk_indices = torch.topk(weights, k=self.topk_heads, dim=-1, largest=True, sorted=False)
            # 使用q norm作为评分
            ...
            del q_norm
            # _, q_head_topk_indices = torch.topk(weights, k=topk_heads, dim=-1, largest=True, sorted=False)

            # 2. 根据索引筛选 q
            # q: (seq, heads, dim) -> 需要选择 (seq, topk_heads, dim)

            # 方法1: 使用 gather (推荐，效率更高)
            # 扩展 indices 到 q 的维度: (seq, topk_heads) -> (seq, topk_heads, 1) -> (seq, topk_heads, dim)
            # topk_indices_expanded = topk_indices.unsqueeze(-1).expand(-1, -1, q.size(-1))
            # q = torch.gather(q, dim=1, index=topk_indices_expanded)
            # weights = torch.gather(weights, dim=1, index=topk_indices)
            # del topk_indices_expanded, topk_indices
...
class MiSALMAAttention(nn.Module):
    """
