...
def test_kernel(
    ...
    def misa_topk_only():
        ...
        logits = misa_mqa_sparse_head_return_logits_interface(q=sparse_head_q, kv=index_k, weights=sparse_head_weights,
                                                             cu_seqlen_ks=cu_seqlen_ks, cu_seqlen_ke=cu_seqlen_ke)
        topk_indices = logits.topk(k=topk_tokens, dim=-1, largest=True, sorted=False).indices.to(torch.int32)
        topk_indices = torch.where((topk_indices >= cu_seqlen_ks.unsqueeze(-1)) & (topk_indices < cu_seqlen_ke.unsqueeze(-1)) ,
                                  topk_indices, -1)
        topk_indices = topk_indices - cu_seqlen_ks.unsqueeze(-1)
        return topk_indices
    ...
    print(f"misa_indexer_topk_reducesum_interface - head_score时间: {misa_head_score_time:.4f} ms")

    # 测量index时间（包含head_score）
    misa_index_time = do_bench(misa_index_only, warmup=WARMUP, rep=REPEAT)
    print(f"misa_indexer_topk_reducesum_interface - index_score时间: {misa_index_time:.4f} ms")
    print(f"misa_indexer_topk_reducesum_interface - index计算时间: {misa_index_time - misa_head_score_time:.4f} ms")

    # 测量topk时间（包含完整流程）
    misa_topk_time = do_bench(misa_topk_only, warmup=WARMUP, rep=REPEAT)
    print(f"misa_indexer_topk_reducesum_interface - 总时间: {misa_topk_time:.4f} ms")
    print(f"misa_indexer_topk_reducesum_interface - topk计算时间: {misa_topk_time - misa_index_time:.4f} ms")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--SK", type=int, default=65536)
